...
# Change Close to numeric; errors='coerce' converts non-numeric to NaN
...
# --- 3. Initialize FinBERT Tokenizer and TF model ---
model_name = 'ProsusAI/finbert'
tokenizer = AutoTokenizer.from_pretrained(model_name)

# Load classification model and extract base BERT
...
base_bert_model = base_bert_model_for_features.bert  # encoder
...
concatenated = tf.keras.layers.concatenate([bert_output_lambda, numerical_input])
...
# Freeze most of BERT, unfreeze last few layers
...
history = model.fit(
    ...
    epochs=20,  # early stopping will limit actual epochs
...
# --- 10. Save model for the demo (minimal change) ---
# This keeps the original training logic intact and only appends a save step.
...
# Save TensorFlow SavedModel (recommended)
model.save(str(SAVEDMODEL_DIR))
print(f"\n✅ SavedModel written to: {SAVEDMODEL_DIR}")

# Also save H5 copy (optional)
model.save(str(H5_PATH))
print(f"✅ H5 model written to: {H5_PATH}")

# Small metadata to document feature order (helps audits/demos)
metadata = {
    "numeric_feature_order": ["sentiment_score_z", "MA_5", "MA_20", "Momentum_1d", "Momentum_5d"],
...
print("ℹ️  Metadata saved alongside the model (demo_metadata.json)")
